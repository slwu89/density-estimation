...
DET_maketree <- function(x, dataset){
  ...
  plot(g,
       ...
       vertex.label = round(tree$ratio,4),
       vertex.shape = node_shape,
       vertex.size = 12,
       vertex.color = node_col,
       vertex.label.color = "black",
       # edge.label = edge_labels,
       edge.label = round(merge(relations, tree, by.x = "to", by.y = "ID")$ratio*ncol(dataset),0),
  ...
  return(
    list(
      ...
      nodes=nodes,
      relations=relations,
      edge_labels=round(merge(relations, tree, by.x = "to", by.y = "ID")$ratio*ncol(dataset),0),
...
#' Process DET output (Simple Version)
#'
#' Process a fitted density estimation tree; return the modified data frame
#' and necessary components to plot the tree with igraph.
#'
...
#'
#' @export
DET_maketree_simple <- function(x){
